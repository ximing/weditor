...
Implement a view that calls `editor.mount` / `editor.unmount`. Reuse `@deditor/preset-docs` and `@deditor/collab`. Do not fork the schema or the OT provider.

```ts
import { Editor } from '@deditor/core'
import { docsPreset } from '@deditor/preset-docs'
import { collabExtension, createWsProvider } from '@deditor/collab'
...
NodeViews for image, task checkbox, and mention live in `@deditor/preset-docs`, so they work without React. Toolbar, find bar, and comment sidebar are React-only in this release — rebuild those in Vue against `editor.commands`, `editor.on('openFind' | 'openComment' | 'openLink' | 'comments' | 'selection')`, and `editor.setEditable`.
